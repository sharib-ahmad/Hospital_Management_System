...
const profile = ref<NurseProfile | null>(null)
...
const saveAvailability = async () => {
  ...
  try {
    ...
    const res = await api.put('/nurses/me', { availability: payload })
...
const avatarLetter = computed(() => {
  return profile.value?.full_name?.charAt(0)?.toUpperCase() || 'N'
})
...
const saveProfile = async () => {
  ...
  isSaving.value = true
  try {
    const res = await api.put('/nurses/me', {
      ...
      <div class="lg:col-span-2 space-y-8">
        ...
        >
          ...
          <p class="text-xs text-gray-500 dark:text-slate-400 font-medium mb-8">
            Configure your working time slots for each day of the week. Patients will only be able to book screening checks during these intervals.
          </p>
