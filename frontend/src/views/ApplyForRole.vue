...
const roles = [
  ...
  },
  {
    id: 'patient',
    title: 'Patient',
    description: 'Access medical records, book appointments, and track your health journey.',
...
const checkStatus = async () => {
  try {
    ...
    if (apps.length > 0) {
      ...
      applicationStatus.value = apps[0].status
    }
  } catch (error) {
...
<template>
  <DashboardLayout>
    ...
    <div v-else-if="hasApplied" class="max-w-2xl mx-auto py-12">
      <div class="glass p-12 rounded-[3rem] text-center border border-white/40 dark:border-white/5 shadow-premium animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div class="w-24 h-24 bg-emerald-50 dark:bg-emerald-900/30 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-sm">
        ...
        <h2 class="text-3xl font-black text-gray-900 dark:text-white mb-4 tracking-tight">Application Submitted!</h2>
        <p class="text-gray-500 dark:text-slate-400 mb-8 font-medium">
          Your application is currently <span class="font-black text-emerald-600 uppercase tracking-widest">{{ applicationStatus }}</span>. 
    ...
    <div v-else-if="!selectedRole" class="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div class="mb-12">
        <h1 class="text-4xl font-black text-gray-900 dark:text-white tracking-tight leading-none">Choose Your Path</h1>
        <p class="text-gray-500 dark:text-slate-400 mt-2 font-medium">Select the role you'd like to perform within the MediFlow ecosystem.</p>
      </div>
      ...
      <div class="grid md:grid-cols-3 gap-10">
        ...
        >
          <div :class="`w-20 h-20 rounded-3xl flex items-center justify-center mb-10 group-hover:scale-110 transition-transform ${role.color}`">
            <svg class="h-10 w-10" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" :d="role.icon" />
            </svg>
          </div>
          <h3 class="text-2xl font-black text-gray-900 dark:text-white mb-4 tracking-tight">{{ role.title }}</h3>
          <p class="text-gray-500 dark:text-slate-400 text-sm font-medium leading-relaxed mb-10">
            {{ role.description }}
          </p>
          <div class="mt-auto">
            <span class="inline-flex items-center gap-2 text-emerald-600 dark:text-emerald-400 font-black uppercase text-[10px] tracking-[0.2em] group-hover:gap-4 transition-all">
              Apply Now
    ...
    <div v-else class="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 group-hover:-translate-x-1 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="3"><path d="M15 19l-7-7 7-7" /></svg>
        Change Role
      </button>

      <div class="glass p-10 sm:p-12 rounded-[3rem] border border-white/40 dark:border-white/5 shadow-premium">
        <div class="flex items-center gap-4 mb-10">
          ...
          <div>
            <h2 class="text-3xl font-black text-gray-900 dark:text-white tracking-tight leading-none">Role Details</h2>
            <p class="text-xs text-gray-500 mt-1 uppercase font-black tracking-widest">Applying as {{ selectedRole }}</p>
          </div>
        </div>
